...
export const Control = (props: Props) => {
    ...
    const on_date_switch_change = (event: React.ChangeEvent<HTMLInputElement>) => {
        console.log('date switch', event.target.checked)
        setDateSwitchChange(event.target.checked)
    }
        ...
        <Stack sx={{ marginBottom: '4px' }} direction="row" spacing={2}>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
                <FormControlLabel control={
                    <Switch {...label}
                        checked={dateSwitch}
                        onChange={on_date_switch_change}
                    />
                } label={'toggle on to include date(s)'} />
                {
                    dateSwitch && <React.Fragment>
                        <DateTimePicker
                            label='start time'
